...
class Supply extends Model
{
    ...
    public function entranceNumbers($id){
        $entrances = \DB::select("select entrance_items.* from entrance_items INNER JOIN entrances ON entrance_items.entrance_id = entrances.id WHERE entrance_items.supply_id = :id AND entrance_items.status = 'Aprobada'", ["id"=>$id]);
        return $entrances;
    }
}
